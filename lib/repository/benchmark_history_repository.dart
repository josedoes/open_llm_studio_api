...
class BenchmarkHistoryRepository {
  ...
  final benchmarkHistoryCache = <String, BenchmarkHistory>{};
  ...
  Future<Either<String, BenchmarkHistory>> create({required BenchmarkHistory model}) async {
    ...
    try {
      ...
      if (result.isRight) {
        ...
        benchmarkHistoryCache[result.right.id ?? ''] = result.right;
      }
      return result;
    } catch (e) {
      return Left('Error');
  ...
  Future<Either<String, BenchmarkHistory?>> read({required String id}) async {
    ...
    final cache = benchmarkHistoryCache[id];
    ...
    if (cache != null) {
      ...
      if (result.isRight) {
        if (result.right != null) {
          ...
          benchmarkHistoryCache[id] = result.right!;
        }
      }
      return result;
    }
  ...
  Future<Either<String, BenchmarkHistory?>> delete({required BenchmarkHistory model}) async {
    try {
      ...
      if (result.isRight) {
        benchmarkHistoryCache.remove(result.right.id ?? '');
      }
      return result;
    } catch (e) {
      return Left('Error');
  ...
  Future<Either<String, List<BenchmarkHistory>>> readAll() async {
    ...
    final cacheList = benchmarkHistoryCache.values.toList();
    ...
    if (cacheList.isNotEmpty) {
      ...
      if (result.isRight) {
        ...
        for (final element in result.right) {
          benchmarkHistoryCache[element.id ?? ''] = element;
        }
      }
      return result;
    }
  ...
  Future<Either<String, List<BenchmarkHistory>>> query(Map<String, dynamic> query) async {
    devLog(tag: tag, message: 'query');

    final cachedList = benchmarkHistoryCache.values.where((e) => e.match(query)).toList();
