...
main() {
  ...
  setUpAll(
    () {
      ...
      GetIt.instance
          .registerSingleton<BenchmarkSpaceService>(BenchmarkSpaceService());
    },
  );
  ...
  group('BenchmarkSpaceService can', () {
    test('Create', () async {
      final exampleModel =
          BenchmarkSpaceOld.fromJson(BenchmarkSpaceOld.exampleJson());
      final setResult = await benchmarkSpaceService.set(model: exampleModel);
    ...
    test('Read', () async {
      ...
      expect(setResult.right, exampleModel);
      final readResult =
          await benchmarkSpaceService.read(id: exampleModel.id ?? '');
    ...
    test('ReadAll', () async {
      final exampleModel =
          BenchmarkSpaceOld.fromJson(BenchmarkSpaceOld.exampleJson());
      final setResult = await benchmarkSpaceService.set(model: exampleModel);
      final setResult2 = await benchmarkSpaceService.set(
          model: exampleModel.copyWith(id: '123'));
      expect(setResult.isRight, true);
      expect(setResult.right, exampleModel);
      final readAllResult = await benchmarkSpaceService.readAll();
